...
<style>
    ...
    .detail-label { @apply text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40; }
    .detail-value { @apply text-[#f0f7f8] font-semibold text-sm mt-1; }
...
@php
    ...
    $hasResult  = $appointment->examResult !== null;
    $isInPatient  = $appointment->inPatient !== null;
    $isOutPatient = $appointment->outPatient !== null;
@endphp
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex items-start justify-between gap-4 mb-8 flex-wrap">
        <div>
            ...
            <h1 class="text-3xl font-bold gradient-text leading-tight">
                Appointment #{{ $appointment->appointment_number }}
            </h1>
            <p class="text-[#CCECEE]/40 text-sm mt-1">
                {{ $appointment->patient->first_name ?? '' }} {{ $appointment->patient->last_name ?? '' }}
                &mdash;
                {{ $appointment->date_time->format('d M Y, h:i A') }}
            </p>
        </div>
    ...
    <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl overflow-hidden shadow-2xl mb-5 fade-in">
        ...
        <div class="flex items-center gap-3 px-6 py-5 border-b border-[#CCECEE]/10 bg-[#03416E]/30">
            ...
            <div>
                <p class="text-xs font-semibold tracking-widest uppercase text-[#CCECEE]/50">Appointment</p>
                <h2 class="text-sm font-bold text-[#f0f7f8]">Schedule Details</h2>
            </div>
        </div>
        ...
        <div class="p-6 grid grid-cols-2 sm:grid-cols-4 gap-6">
            ...
            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Appt. No.</p>
                <p class="text-[#f0f7f8] font-mono font-bold text-lg leading-none mt-1">{{ $appointment->appointment_number }}</p>
            </div>

            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Date</p>
                <p class="text-[#f0f7f8] font-semibold text-sm mt-1">{{ $appointment->date_time->format('d M Y') }}</p>
                <p class="text-[#CCECEE]/40 text-xs">{{ $appointment->date_time->format('l') }}</p>
            </div>

            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Time</p>
                <p class="text-[#f0f7f8] font-semibold text-sm mt-1">{{ $appointment->date_time->format('h:i A') }}</p>
            </div>
        ...
        {{-- Staff strip --}}
        <div class="mx-6 mb-6 bg-[#CCECEE]/5 border border-[#CCECEE]/10 rounded-xl px-4 py-3 flex items-center gap-3">
            <svg class="w-4 h-4 text-[#CCECEE]/40 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
            </svg>
            <span class="text-xs font-semibold uppercase tracking-wider text-[#CCECEE]/40">Assigned Staff</span>
            <span class="text-[#CCECEE]/70 font-mono text-sm font-semibold">{{ $appointment->staff_number }}</span>
        </div>
    ...
    <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl overflow-hidden shadow-2xl mb-5 fade-in">
        ...
        <div class="flex items-center gap-3 px-6 py-5 border-b border-[#CCECEE]/10 bg-[#03416E]/20">
            ...
            <div class="flex-1">
                <p class="text-xs font-semibold tracking-widest uppercase text-[#CCECEE]/50">Patient</p>
                <h2 class="text-sm font-bold text-[#f0f7f8]">Patient Information</h2>
            </div>
            @if ($appointment->patient)
                <a href="{{ route('patients.show', $appointment->patient_number) }}"
        ...
        <div class="p-6 grid grid-cols-2 sm:grid-cols-3 gap-6">
            ...
            <div class="flex flex-col gap-1">
                ...
                <p class="text-[#f0f7f8] font-mono font-bold mt-1">{{ $appointment->patient_number }}</p>
            </div>

            @if ($appointment->patient)
                <div class="flex flex-col gap-1">
                    <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Date of Birth</p>
                    <p class="text-[#CCECEE]/70 text-sm mt-1">{{ $appointment->patient->date_of_birth->format('d M Y') }}</p>
                </div>

                <div class="flex flex-col gap-1">
                    <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Telephone</p>
                    <p class="text-[#CCECEE]/70 text-sm mt-1">{{ $appointment->patient->telephone_number }}</p>
                </div>

                @if ($appointment->patient->localDoctor)
                    <div class="flex flex-col gap-1">
                        <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Referred By</p>
                        <p class="text-[#CCECEE]/70 text-sm mt-1">{{ $appointment->patient->localDoctor->full_name }}</p>
                    </div>
                @endif
            @endif
    ...
    <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl overflow-hidden shadow-2xl mb-5 fade-in">
        ...
        <div class="flex items-center gap-3 px-6 py-5 border-b border-[#CCECEE]/10
             {{ $hasResult ? 'bg-emerald-500/5' : 'bg-amber-500/5' }}">
            <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0
                 {{ $hasResult ? 'bg-emerald-500/15 border border-emerald-500/20 text-emerald-400' : 'bg-amber-500/15 border border-amber-500/20 text-amber-400' }}">
                @if ($hasResult)
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
        ...
        @if ($hasResult)
            <div class="p-6 grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <div class="mt-1">
                        @if ($appointment->examResult->result === 'Out-patient')
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <p class="text-[#f0f7f8] font-semibold text-sm mt-1">
                        {{ $appointment->examResult->examined_date->format('d M Y') }}
                    </p>
                </div>

                <div class="flex flex-col gap-1">
                    <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Classification</p>
                    <p class="text-[#CCECEE]/70 text-sm mt-1">
                        @if ($isInPatient)  Admitted (In-patient)
                        @elseif ($isOutPatient) Discharged (Out-patient)
        ...
        @else
            {{-- No result yet — show record result form if appointment is past --}}
            <div class="p-6">
                @if (!$isUpcoming)
                    <p class="text-[#CCECEE]/50 text-sm mb-5">
                        This appointment has passed. Record the exam result below.
                    </p>
                    <form action="{{ route('appointments.record_result', $appointment->appointment_number) }}"
                          method="POST" id="resultForm">
                        @csrf
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <select id="result" name="result"
                                        class="bg-[#021829] border border-[#CCECEE]/20 text-[#f0f7f8] text-sm rounded-xl px-4 py-2.5 w-full outline-none focus:border-[#CCECEE]/60 focus:ring-2 focus:ring-[#CCECEE]/10 transition">
                                    <option value="" disabled selected>— Select result —</option>
                                    <option value="Out-patient">Out-patient</option>
                                    <option value="WaitingList">Waiting List (Admit)</option>
                                </select>
                                @error('result')
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input type="date" id="examined_date" name="examined_date"
                        ...
                        <button type="submit" id="resultBtn"
                                class="inline-flex items-center gap-2 bg-[#03416E] hover:bg-[#CCECEE] hover:text-[#021829] text-[#CCECEE] border border-[#CCECEE]/30 text-sm font-semibold px-5 py-2.5 rounded-xl transition-all duration-200 hover:-translate-y-0.5">
                ...
                @else
                    <div class="text-center py-6">
                        <p class="text-[#CCECEE]/40 text-sm">No exam result recorded yet.</p>
                        <p class="text-[#CCECEE]/25 text-xs mt-1">Result can be recorded after the appointment date passes.</p>
                    </div>
                @endif
            </div>
        @endif
    ...
    @if ($isInPatient || $isOutPatient)
    <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl overflow-hidden shadow-2xl mb-5 fade-in">

        <div class="flex items-center gap-3 px-6 py-5 border-b border-[#CCECEE]/10
             {{ $isInPatient ? 'bg-violet-500/5' : 'bg-sky-500/5' }}">
            <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0
                 {{ $isInPatient ? 'bg-violet-500/15 border border-violet-500/20 text-violet-400' : 'bg-sky-500/15 border border-sky-500/20 text-sky-400' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <div>
                ...
                <h2 class="text-sm font-bold text-[#f0f7f8]">
                    {{ $isInPatient ? 'In-Patient Record' : 'Out-Patient Record' }}
                </h2>
            </div>
        </div>
        ...
        <div class="p-6 grid grid-cols-2 sm:grid-cols-4 gap-6">
            <div class="flex flex-col gap-1">
                ...
                <p class="text-[#f0f7f8] font-semibold text-sm mt-1">{{ $appointment->inPatient->ward_number }}</p>
            </div>
            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Bed</p>
                <p class="text-[#f0f7f8] font-semibold text-sm mt-1">{{ $appointment->inPatient->bed_number }}</p>
            </div>
            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Expected Stay</p>
                <p class="text-[#f0f7f8] font-semibold text-sm mt-1">{{ $appointment->inPatient->expected_stay }} days</p>
            </div>
            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Date Placed</p>
                <p class="text-[#f0f7f8] font-semibold text-sm mt-1">{{ $appointment->inPatient->date_placed->format('d M Y') }}</p>
            </div>
            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Expected Leave</p>
                <p class="text-[#CCECEE]/70 text-sm mt-1">
                    {{ $appointment->inPatient->date_leave?->format('d M Y') ?? '—' }}
                </p>
            </div>
            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Actual Leave</p>
                @if ($appointment->inPatient->actual_leave)
                    <p class="text-[#f0f7f8] font-semibold text-sm mt-1">{{ $appointment->inPatient->actual_leave->format('d M Y') }}</p>
                @else
        ...
        <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div class="flex flex-col gap-1">
                ...
                <p class="text-[#f0f7f8] font-mono font-bold mt-1">{{ $appointment->outPatient->patient_number }}</p>
            </div>
            <div class="flex flex-col gap-1">
                <p class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/40">Appointment Date</p>
                <p class="text-[#f0f7f8] font-semibold text-sm mt-1">
                    {{ $appointment->outPatient->appointment_date_time->format('d M Y, h:i A') }}
                </p>
            </div>
        </div>
        @endif
    ...
    <div class="flex items-center justify-between gap-3 flex-wrap fade-in">
        ...
        <form action="{{ route('appointments.destroy', $appointment->appointment_number) }}"
              method="POST"
              onsubmit="return confirm('Delete appointment #{{ $appointment->appointment_number }}?\n\nThis cannot be undone.')">
            @csrf
            @method('DELETE')
            <button type="submit"
