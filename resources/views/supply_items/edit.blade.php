...
<div class="max-w-3xl mx-auto px-4 sm:px-6 py-10">
    ...
    {{-- ── Back Link ── --}}
    <a href="{{ route('supply_items.show', $supplyItem->item_number) }}"
    ...
    <div class="mb-7 anim-fade-up">
        ...
        <h1 class="text-3xl font-bold text-white tracking-tight mb-3">Edit Supply Item</h1>
        {{-- Item ID pill --}}
        <span class="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/25
                     text-amber-400 text-xs font-semibold rounded-full px-3.5 py-1.5">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"/>
            </svg>
            Item ID: <span class="font-mono tracking-wider">{{ $supplyItem->item_number }}</span>
        </span>
    </div>
    ...
    <form action="{{ route('supply_items.update', $supplyItem->item_number) }}"
          method="POST"
          id="supplyForm"
          class="space-y-5">
        @csrf
        ...
        <div class="bg-wm-card border border-wm-navy/60 rounded-2xl overflow-hidden
            ...
            <div class="px-6 py-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input
                        type="text"
                        value="{{ $supplyItem->item_number }}"
                        disabled
                        class="w-full bg-wm-dark/60 border border-wm-navy/40 rounded-xl text-slate-500
                               text-sm px-4 py-2.5 focus:outline-none"
                    >
                    <p class="text-xs text-slate-600">Item ID cannot be changed after creation.</p>
                </div>

                {{-- Item Name --}}
                <div class="flex flex-col gap-1.5">
                    <label for="item_name" class="text-xs font-semibold tracking-wide uppercase text-slate-400">
                        Item Name <span class="text-amber-400">*</span>
                    </label>
                    <input
                        type="text"
                        id="item_name"
                    ...
                    <textarea
                        ...
                        class="w-full bg-wm-dark border rounded-xl text-white text-sm placeholder-slate-600
                               ...
                               {{ $errors->has('description')
                                    ? 'border-red-500/60 focus:border-red-500 focus:ring-2 focus:ring-red-500/15'
                                    : 'border-wm-navy/70 focus:border-amber-400/60 focus:ring-2 focus:ring-amber-400/10' }}"
                    >{{ old('description', $supplyItem->description) }}</textarea>
                    @error('description')
                        <p class="flex items-center gap-1.5 text-xs text-red-400">
                            <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
            ...
            <div class="px-6 py-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="flex flex-col gap-1.5">
                    <label for="quantity_in_stock" class="text-xs font-semibold tracking-wide uppercase text-slate-400">
                        Quantity In Stock <span class="text-amber-400">*</span>
                    </label>
                    ...
                    <div id="stockStatus" class="mt-0.5"></div>
                ...
                <div class="flex flex-col gap-1.5">
                    <label for="reorder_level" class="text-xs font-semibold tracking-wide uppercase text-slate-400">
                        Reorder Level <span class="text-amber-400">*</span>
                    </label>
                    <div class="flex rounded-xl overflow-hidden border transition-all duration-200
                        ...
                        <input
                            ...
                            id="reorder_level"
                            name="reorder_level"
                            value="{{ old('reorder_level', $supplyItem->reorder_level) }}"
                            min="0"
                            oninput="updateStockStatus()"
                            class="flex-1 min-w-0 bg-wm-dark text-white text-sm placeholder-slate-600
                                   px-3 py-2.5 focus:outline-none"
                ...
                <div class="flex flex-col gap-1.5 sm:col-span-2">
                    <label for="cost_per_unit" class="text-xs font-semibold tracking-wide uppercase text-slate-400">
                        Cost Per Unit <span class="text-amber-400">*</span>
                    </label>
                    <div class="flex rounded-xl overflow-hidden border transition-all duration-200
                        ...
                        <input
                            ...
                            id="cost_per_unit"
                            name="cost_per_unit"
                            value="{{ old('cost_per_unit', $supplyItem->cost_per_unit) }}"
                            min="0"
                            step="0.01"
                            placeholder="0.00"
                            class="flex-1 min-w-0 bg-wm-dark text-white text-sm placeholder-slate-600
        ...
        <div class="bg-wm-card border border-wm-navy/60 rounded-2xl overflow-hidden
            ...
            <div class="px-6 py-6">
                <div class="flex flex-col gap-1.5">
                    <label for="supplier_number" class="text-xs font-semibold tracking-wide uppercase text-slate-400">
                        Supplier <span class="text-amber-400">*</span>
                    </label>
                    <select
                        id="supplier_number"
                    ...
                    >
                        ...
                        @foreach ($suppliers as $supplier)
                            <option
                                ...
                                {{ old('supplier_number', $supplyItem->supplier_number) == $supplier->supplier_number ? 'selected' : '' }}
                            >
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-4
                ...
                <span class="hidden sm:flex items-center gap-1.5 text-xs text-slate-500">
                    ...
                    Last updated: {{ $supplyItem->updated_at->format('M d, Y h:i A') }}
                </span>
...
<script>
    ...
    // Run on load so indicator shows immediately
    updateStockStatus();
