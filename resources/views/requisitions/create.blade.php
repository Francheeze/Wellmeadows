...
<style>
    ...
    /* Custom select arrow in hospital cyan */
    .wm-select {
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold gradient-text leading-tight">New Requisition</h1>
    </div>
    ...
    <form action="{{ route('requisitions.store') }}" method="POST" id="reqForm">
        ...
        <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl overflow-hidden shadow-2xl mb-5 fade-in">
            ...
            <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="flex flex-col gap-1.5">
                    <label for="requisition_number" class="text-xs font-semibold tracking-wider uppercase text-[#CCECEE]/60">
                        Requisition Number <span class="text-[#CCECEE]">*</span>
                    </label>
                    <input type="number" id="requisition_number" name="requisition_number"
                           value="{{ old('requisition_number') }}" placeholder="e.g. 3001" min="1"
                           class="bg-[#021829] border {{ $errors->has('requisition_number') ? 'border-red-400/60 ring-2 ring-red-400/10' : 'border-[#CCECEE]/20 focus:border-[#CCECEE]/60 focus:ring-2 focus:ring-[#CCECEE]/10' }} text-[#f0f7f8] placeholder-[#CCECEE]/30 text-sm rounded-xl px-4 py-2.5 w-full outline-none transition">
                    <p class="text-xs text-[#CCECEE]/40">Must be a unique integer.</p>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="date" id="date_ordered" name="date_ordered"
                           value="{{ old('date_ordered', date('Y-m-d')) }}"
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="number" id="staff_number" name="staff_number"
                           value="{{ old('staff_number') }}" placeholder="e.g. 101" min="1"
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="number" id="ward_number" name="ward_number"
                           value="{{ old('ward_number') }}" placeholder="e.g. 5" min="1"
        ...
        <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl overflow-hidden shadow-2xl mb-5 fade-in">
            <div class="flex items-center gap-3 px-6 py-5 border-b border-[#CCECEE]/10 bg-emerald-500/5">
                ...
                <div>
                    <p class="text-xs font-semibold tracking-widest uppercase text-[#CCECEE]/50">Step 2 — Optional</p>
            ...
            <div class="p-6">
                ...
                <div class="flex flex-col gap-3" id="drugItemsList">
                    @if (old('drug_items'))
                        @foreach (old('drug_items') as $index => $drug)
                        <div class="grid grid-cols-[1fr_140px_36px] gap-3 items-end bg-[#021829]/60 border border-[#CCECEE]/10 rounded-xl p-3" id="drug-row-{{ $index }}" style="animation:rowSlideIn .2s ease both;">
                            <div>
                                <p class="text-xs font-semibold uppercase tracking-wider text-[#CCECEE]/40 mb-1.5">Drug</p>
                                <select name="drug_items[{{ $index }}][drug_number]"
                                        class="wm-select bg-[#021829] border border-[#CCECEE]/20 text-[#f0f7f8] text-sm rounded-xl px-3 py-2 w-full outline-none focus:border-[#CCECEE]/50 transition">
                            ...
                            <div>
                                ...
                                <input type="number" name="drug_items[{{ $index }}][quantity_required]"
                                       value="{{ $drug['quantity_required'] }}" min="1" placeholder="0"
                                       class="bg-[#021829] border border-[#CCECEE]/20 text-[#f0f7f8] placeholder-[#CCECEE]/30 text-sm rounded-xl px-3 py-2 w-full outline-none focus:border-[#CCECEE]/50 transition">
                            </div>
                            <button type="button" onclick="removeRow('drug-row-{{ $index }}', 'drug')"
                ...
                {{-- Empty state --}}
                <div id="drugEmptyState" class="text-center py-8 border-2 border-dashed border-[#CCECEE]/10 rounded-xl text-[#CCECEE]/30 text-sm {{ old('drug_items') ? 'hidden' : '' }}">
                    <svg class="w-8 h-8 mx-auto mb-2 opacity-30" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/>
                    </svg>
                    No drug items added yet. Click <strong class="text-[#CCECEE]/50">Add Drug</strong> to begin.
                </div>

                @error('items')
                    <div class="flex items-center gap-2 bg-red-400/8 border border-red-400/25 text-red-400 text-sm font-medium px-4 py-3 rounded-xl mt-3">
        ...
        <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl overflow-hidden shadow-2xl fade-in">
            <div class="flex items-center gap-3 px-6 py-5 border-b border-[#CCECEE]/10 bg-violet-500/5">
                ...
                <div>
                    <p class="text-xs font-semibold tracking-widest uppercase text-[#CCECEE]/50">Step 3 — Optional</p>
            ...
            <div class="p-6">
                ...
                <div class="flex flex-col gap-3" id="supplyItemsList">
                    @if (old('supply_items'))
                        @foreach (old('supply_items') as $index => $supply)
                        <div class="grid grid-cols-[1fr_140px_36px] gap-3 items-end bg-[#021829]/60 border border-[#CCECEE]/10 rounded-xl p-3" id="supply-row-{{ $index }}" style="animation:rowSlideIn .2s ease both;">
                            <div>
                                <p class="text-xs font-semibold uppercase tracking-wider text-[#CCECEE]/40 mb-1.5">Item</p>
                                <select name="supply_items[{{ $index }}][item_number]"
                                        class="wm-select bg-[#021829] border border-[#CCECEE]/20 text-[#f0f7f8] text-sm rounded-xl px-3 py-2 w-full outline-none focus:border-[#CCECEE]/50 transition">
                                    <option value="" disabled>— Select item —</option>
                                    @foreach ($supplyItems as $supplyItem)
                                        <option value="{{ $supplyItem->item_number }}" {{ $supply['item_number'] == $supplyItem->item_number ? 'selected' : '' }}>
                                            #{{ $supplyItem->item_number }} — {{ $supplyItem->item_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <p class="text-xs font-semibold uppercase tracking-wider text-[#CCECEE]/40 mb-1.5">Qty Required</p>
                                <input type="number" name="supply_items[{{ $index }}][quantity_required]"
                                       value="{{ $supply['quantity_required'] }}" min="1" placeholder="0"
                                       class="bg-[#021829] border border-[#CCECEE]/20 text-[#f0f7f8] placeholder-[#CCECEE]/30 text-sm rounded-xl px-3 py-2 w-full outline-none focus:border-[#CCECEE]/50 transition">
                            </div>
                            <button type="button" onclick="removeRow('supply-row-{{ $index }}', 'supply')"
                                    class="flex items-center justify-center w-9 h-9 rounded-lg bg-red-400/8 border border-red-400/20 text-red-400/60 hover:bg-red-400/15 hover:text-red-400 transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                ...
                <div id="supplyEmptyState" class="text-center py-8 border-2 border-dashed border-[#CCECEE]/10 rounded-xl text-[#CCECEE]/30 text-sm {{ old('supply_items') ? 'hidden' : '' }}">
                    <svg class="w-8 h-8 mx-auto mb-2 opacity-30" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10"/>
                    </svg>
                    No supply items added yet. Click <strong class="text-[#CCECEE]/50">Add Item</strong> to begin.
                </div>
            </div>
...
<script>
    ...
    let drugIndex   = {{ old('drug_items') ? count(old('drug_items')) : 0 }};
    let supplyIndex = {{ old('supply_items') ? count(old('supply_items')) : 0 }};
    ...
    const rowCls    = 'grid gap-3 items-end bg-[#021829]/60 border border-[#CCECEE]/10 rounded-xl p-3';
    ...
    function addDrugRow() {
        ...
        const empty = document.getElementById('drugEmptyState');
        const id    = 'drug-row-' + drugIndex;
        const div   = document.createElement('div');
        div.className = rowCls + ' grid-cols-[1fr_140px_36px]';
        div.id = id;
    ...
    function addSupplyRow() {
        ...
        const empty = document.getElementById('supplyEmptyState');
