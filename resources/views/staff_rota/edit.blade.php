...
.form-group input:disabled { background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.4); cursor: not-allowed; border-color: rgba(255,255,255,0.05); }
...
<div class="wb-page">
    ...
    <div class="form-card">
        ...
        <form action="{{ route('staff-rota.update', $staffRota->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <select name="ward_number" required>
                    ...
                    @foreach($wards as $ward)
                        <option value="{{ $ward->ward_number }}"
                            {{ old('ward_number', $staffRota->ward_number) == $ward->ward_number ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="shift" required>
                    <option value="Early" {{ old('shift', $staffRota->shift) == 'Early' ? 'selected' : '' }}>Early</option>
                    <option value="Late"  {{ old('shift', $staffRota->shift) == 'Late'  ? 'selected' : '' }}>Late</option>
                    <option value="Night" {{ old('shift', $staffRota->shift) == 'Night' ? 'selected' : '' }}>Night</option>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" name="week_start_date"
                       value="{{ old('week_start_date', \Carbon\Carbon::parse($staffRota->week_start_date ?? $staffRota->weekstartdate)->format('Y-m-d')) }}"
                       required>
            </div>

            <div class="form-actions">
                <button type="submit" class="btn-save">Update Allocation</button>
