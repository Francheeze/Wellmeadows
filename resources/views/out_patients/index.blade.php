...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex items-end justify-between mb-10 gap-4 flex-wrap">
        <div>
            ...
            <h1 class="text-4xl font-bold gradient-text leading-tight">Out-Patients</h1>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-7">
        ...
        <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl px-5 py-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-[#f0f7f8] leading-none">{{ $outPatients->total() }}</p>
        ...
        <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl px-5 py-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-[#f0f7f8] leading-none">
                    {{ $outPatients->getCollection()->filter(fn($op) => $op->appointment_date_time->isFuture())->count() }}
                </p>
        ...
        <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl px-5 py-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-[#f0f7f8] leading-none">
                    {{ $outPatients->getCollection()->filter(fn($op) => $op->appointment_date_time->isPast())->count() }}
                </p>
                <p class="text-xs text-[#CCECEE]/50 font-medium mt-1">Completed</p>
            </div>
        </div>
    ...
    <form method="GET" action="{{ route('out_patients.index') }}"
        ...
        <div class="bg-[#032d4f] border border-[#CCECEE]/20 rounded-xl px-4 py-2.5 text-sm font-semibold text-[#CCECEE]/60 whitespace-nowrap ml-auto">
            Total: <span class="text-[#CCECEE]">{{ $outPatients->total() }}</span>
        </div>
    </form>
    ...
    <div class="bg-[#032d4f] border border-[#CCECEE]/15 rounded-2xl overflow-hidden shadow-2xl fade-in">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-[#CCECEE]/10 bg-[#03416E]/30">
                        ...
                        <th class="px-6 py-3.5 text-left text-xs font-semibold text-[#CCECEE]/60 uppercase tracking-wider">Appointment Date & Time</th>
                    ...
                    <tr class="hover:bg-[#03416E]/20 transition duration-100">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-1.5 text-[#CCECEE]/70 text-sm">
                                ...
                                {{ $op->appointment_date_time->format('d M Y') }}
                            </div>
                            <div class="text-[#CCECEE]/40 text-xs mt-0.5 pl-5">
                                {{ $op->appointment_date_time->format('h:i A') }}
                            </div>
                        </td>

                        {{-- Exam Result --}}
                        <td class="px-6 py-4">
                            @if ($op->appointment?->examResult)
                        ...
                        <td class="px-6 py-4">
                            @if ($op->appointment_date_time->isFuture())
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                {{-- View --}}
                                <a href="{{ route('out_patients.show', $op->appointment_number) }}"
                                   title="View"
                                ...
                                {{-- Edit --}}
                                <a href="{{ route('out_patients.edit', $op->appointment_number) }}"
                                   title="Edit"
                                ...
                                {{-- Delete --}}
                                <form action="{{ route('out_patients.destroy', $op->appointment_number) }}"
                                      method="POST"
                                      onsubmit="return confirm('Delete out-patient record #{{ $op->appointment_number }}?\n\nThis cannot be undone.')">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="flex items-center justify-between px-6 py-4 border-t border-[#CCECEE]/10 flex-wrap gap-3">
            <p class="text-xs text-[#CCECEE]/40">
                Showing <span class="font-semibold text-[#CCECEE]/70">{{ $outPatients->firstItem() }}</span>
                to <span class="font-semibold text-[#CCECEE]/70">{{ $outPatients->lastItem() }}</span>
                of <span class="font-semibold text-[#CCECEE]/70">{{ $outPatients->total() }}</span> records
            </p>
            {{ $outPatients->links() }}
        </div>
        @endif
