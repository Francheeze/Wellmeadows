...
@section('title', 'Add Pharmaceutical Item')

@push('styles')
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<style>
    /* Custom select arrow — Tailwind has no bg-image utility for this */
    .wm-select {
        appearance: none;
...
<div class="max-w-[860px] mx-auto px-6 py-10">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-white tracking-tight">Add New Drug</h1>
    </div>
    ...
    <form action="{{ route('pharmaceutical_items.store') }}" method="POST" id="pharmaForm">
        @csrf
        ...
        <div class="bg-wm-card border border-wm-navy/50 rounded-2xl overflow-hidden shadow-xl shadow-black/30 mb-5">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-wm-navy/40 bg-wm-navy/20">
                ...
                <div>
                    <p class="text-[10px] font-semibold tracking-widest uppercase text-slate-500">Step 1</p>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label for="drug_number" class="text-[11px] font-semibold tracking-wider uppercase text-slate-400">
                            Drug Number <span class="text-wm-cyan">*</span>
                        </label>
                        <input type="number" id="drug_number" name="drug_number"
                               value="{{ old('drug_number') }}"
                               placeholder="e.g. 1001"
                               class="w-full bg-wm-dark border rounded-xl text-slate-200 text-sm px-4 py-2.5 placeholder-slate-600
                                      focus:outline-none focus:ring-2 transition-all duration-200
                                      {{ $errors->has('drug_number')
                        ...
                        <span class="text-[11px] text-slate-500">Unique numeric ID for this drug.</span>
                        @error('drug_number')
                    ...
                    <div class="flex flex-col gap-1.5 sm:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3"
                                  placeholder="Brief description of the drug, its use, or any notes…"
        ...
        <div class="bg-wm-card border border-wm-navy/50 rounded-2xl overflow-hidden shadow-xl shadow-black/30 mb-5">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-wm-navy/40 bg-wm-navy/20">
                ...
                <div>
                    <p class="text-[10px] font-semibold tracking-widest uppercase text-slate-500">Step 2</p>
                        ...
                        <select id="method_of_administration" name="method_of_administration"
                                class="wm-select w-full bg-wm-dark border rounded-xl text-slate-200 text-sm px-4 py-2.5 cursor-pointer
                                       ...
                                       {{ $errors->has('method_of_administration')
                                           ? 'border-red-400/50 focus:border-red-400/50 focus:ring-red-400/10'
                                           : 'border-wm-navy/60 focus:border-wm-cyan/50 focus:ring-wm-cyan/10' }}">
                            <option value="" disabled {{ old('method_of_administration') ? '' : 'selected' }}>
                                — Select method —
                            </option>
                            @foreach ([
                                ...
                                <option value="{{ $method }}" {{ old('method_of_administration') === $method ? 'selected' : '' }}>
                                    {{ $method }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="bg-wm-card border border-wm-navy/50 rounded-2xl overflow-hidden shadow-xl shadow-black/30 mb-5">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-wm-navy/40 bg-wm-navy/20">
                ...
                <div>
                    <p class="text-[10px] font-semibold tracking-widest uppercase text-slate-500">Step 3</p>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="flex items-stretch border rounded-xl overflow-hidden transition-all duration-200
                            ...
                            <input type="number" id="quantity_in_stock" name="quantity_in_stock"
                                   value="{{ old('quantity_in_stock', 0) }}" min="0" placeholder="0"
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="flex items-stretch border rounded-xl overflow-hidden transition-all duration-200
                            ...
                            <input type="number" id="reorder_level" name="reorder_level"
                                   value="{{ old('reorder_level', 0) }}" min="0" placeholder="0"
                                   class="flex-1 min-w-0 bg-wm-dark text-slate-200 text-sm px-3 py-2.5 placeholder-slate-600 focus:outline-none">
                            <span class="flex items-center px-3 bg-wm-navy/40 border-l border-wm-navy/60 text-slate-400 text-[12px] font-medium shrink-0">
                                units
                            </span>
                    ...
                    <div class="flex flex-col gap-1.5 sm:col-span-2">
                        ...
                        <div class="flex items-stretch border rounded-xl overflow-hidden transition-all duration-200
                            ...
                            <input type="number" id="cost_per_unit" name="cost_per_unit"
                                   value="{{ old('cost_per_unit') }}" min="0" step="0.01" placeholder="0.00"
                                   class="flex-1 min-w-0 bg-wm-dark text-slate-200 text-sm px-3 py-2.5 placeholder-slate-600 focus:outline-none">
                            <span class="flex items-center px-3 bg-wm-navy/40 border-l border-wm-navy/60 text-slate-400 text-[12px] font-medium shrink-0">
                                PHP
                            </span>
        ...
        <div class="bg-wm-card border border-wm-navy/50 rounded-2xl overflow-hidden shadow-xl shadow-black/30">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-wm-navy/40 bg-wm-navy/20">
                ...
                <div>
                    <p class="text-[10px] font-semibold tracking-widest uppercase text-slate-500">Step 4</p>
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-wm-navy/40 bg-wm-navy/10 flex-wrap">
                <a href="{{ route('pharmaceutical_items.index') }}"
