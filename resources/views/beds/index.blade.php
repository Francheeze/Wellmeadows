...
.status-avail { background: rgba(16,185,129,0.15); color: #6ee7b7; border: 1px solid rgba(16,185,129,0.25); }
.status-occ { background: rgba(245,158,11,0.15); color: #fcd34d; border: 1px solid rgba(245,158,11,0.25); }
...
<div class="wb-page">
    ...
    <div class="attr-grid">
        <div class="attr-card">
            ...
            <div class="attr-label">Total beds in database</div>
            <div class="attr-value">{{ $totalBeds }}</div>
            <div class="attr-sub">All registered</div>
        </div>
        <div class="attr-card">
            <div class="attr-icon"><i class="ti ti-circle-check"></i></div>
            <div class="attr-label">Available open beds</div>
            <div class="attr-value">{{ $availableBeds }}</div>
            <div class="attr-sub">Ready to assign</div>
        </div>
        <div class="attr-card">
            <div class="attr-icon"><i class="ti ti-user"></i></div>
            <div class="attr-label">Occupied beds taken</div>
            <div class="attr-value">{{ $occupiedBeds }}</div>
            <div class="attr-sub">Currently in use</div>
        </div>
        <div class="attr-card">
            <div class="attr-icon"><i class="ti ti-building-hospital"></i></div>
            <div class="attr-label">Wards covered with beds</div>
            <div class="attr-value">{{ $wardsCovered }}</div>
            <div class="attr-sub">Wards assigned</div>
        </div>
    </div>

    <div class="table-card">
        <div class="tc-header">
            <span class="tc-title">Bed list</span>
            <div class="tc-right">
                ...
                <select class="tc-select" id="statusFilter" onchange="filterBeds()">
                    <option value="all">All statuses</option>
                    <option value="available">Available</option>
                    <option value="occupied">Occupied</option>
                </select>
                <a href="{{ route('beds.create') }}" class="btn-add">+ Add bed</a>
            </div>
        </div>
            ...
            <tbody id="bed-tbody">
                ...
                <tr class="bed-row" data-ward="{{ $bed->ward_number }}" data-status="{{ $bed->status }}">
                    ...
                    <td>
                        ...
                        @else
                            <span class="status-badge status-occ">Occupied</span>
                        @endif
                    </td>
                    <td>
                ...
                @empty
                <tr class="empty-state"><td colspan="4">No beds found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
function filterBeds() {
    ...
    document.querySelectorAll('.bed-row').forEach(row => {
        ...
        const ms = status === 'all' || row.dataset.status === status;
